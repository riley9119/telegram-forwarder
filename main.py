...
# Optional default topic for TARGET_CHAT (top message id of the thread)
...
def current_slot_idx() -> int:
    ...
    return (now_local.hour // 2) % 12   # 12 slots per day, every 2 hours
...
async def copy_message(
    ...
    """
    Forward a message into TARGET_CHAT.
    If topic_id (top message id) is provided (or configured via env),
    it will be posted inside that forum topic.
    """
    await ensure_connected()

    dest = to_peer or TARGET_CHAT
    top_id = topic_id if topic_id is not None else TARGET_TOPIC_ID

    # Use ForwardMessages with top_msg_id (works across Telethon versions)
    kwargs = {}
    if top_id:
        kwargs["top_msg_id"] = int(top_id)

    # Raw request is fine, but the helper also works; raw keeps it explicit:
...
async def hook(request: Request):
    ...
    # Choose which message to forward
    ...
    # Optional per-call topic override
    topic_override = None
